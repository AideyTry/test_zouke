...
module.exports = class Pretreatment{
    ...
    async _resolveCollection(name, { resolveMode = 'R',isSai = false } = {}){
        const collection = await dbclient.collections.get(name);
    ...
    async run(){
        ...
        await this._resolveCollection('zk_hotels', { isSai: true });
        console.log('<-- zk_hotel pretreatment end');
        console.log('--> sp_hotel pretreatment start');
        await this._resolveCollection('sp_hotels', { resolveMode: 'I' });   
